...
**模型分档 HIGH/MID**: 模型能力分级。HIGH 用于复杂推理/主 Agent（roomdoor, veteran, librarian, qiqi），MID 用于简单任务（ccy, update）。各 Agent 可配置 fallback_model 降级。
_Avoid_: "大模型/小模型"（不精确——模型 ID 可能相同，只有档位标志不同）

**L3 升级** (Level 3 Escalation): 当 `roomdoor` 识别到高风险/高价值场景（金额 > 5000 元、含税法关键词、合同关键词、重大人生决策），显式 `@veteran`（老江湖）升级处理。
_Avoid_: "报错"、"升级"（太泛）
...
- 房间门团队由 roomdoor（主）、veteran（升级）、librarian、qiqi、ccy、update（6 个 Agent）组成
- update Agent 是记忆系统 + 项目元信息的 single-writer
- L3 升级是 roomdoor → veteran 的保卫逻辑
